	...
	while(1)
	{
		...
		if(current_ms-last_send_ms >= 100) //sending rate controlled here one message every 100ms (10Hz)
		{
			...
			joystickADC = adc_read(0);
		   	compValueV = (joystickADC/(uint32_t)5);
			sendDataByte1 = compValueV;
			
			//Horizontal
			joystickADC = adc_read(1);
			compValueH = (joystickADC/(uint32_t)5);
			sendDataByte2 = compValueH;

			sprintf(serial_string, "Send: 1:%4d, 2:%4d , 3:%4d , 4:%4d \n", sendDataByte1, sendDataByte2, dataByte3, dataByte4);
